...
describe('Graph', () => {
  ...
  it('safely compares null and numerical values', () => {
    ...
    expect(Graph.nullSafeLessThan(0, null)).toBeTruthy();
    expect(Graph.nullSafeLessThan(1, null)).toBeTruthy();
    expect(Graph.nullSafeLessThan(-1, null)).toBeTruthy();
    expect(Graph.nullSafeLessThan(null, 0)).toBeFalsy();
    expect(Graph.nullSafeLessThan(null, 1)).toBeFalsy();
    expect(Graph.nullSafeLessThan(null, -1)).toBeFalsy();
  });
  it('adds null and numerical values safely', () => {
    expect(Graph.nullSafeSum(null, 0)).toBeNull();
    expect(Graph.nullSafeSum(0, -2)).toBe(-2);
    expect(Graph.nullSafeSum(-2, -2)).toBe(-4);
    expect(Graph.nullSafeSum(null, null)).toBeNull();
  });
  it('relaxes edges', () => {
    const a = new Vertex('a');
    const b = new Vertex('b');
